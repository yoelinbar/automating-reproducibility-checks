# Plan — Independent Reproducibility Checks of 4 Published Papers

## Context

This is an independent reproducibility-audit task. You will be given a list of statistical claims from 4 published social/behavioural-science papers, and your job is to obtain each paper's data and code, rerun the analysis, and judge whether each claim reproduces — using the fixed rubric defined in this plan.
...
|---|---|---|
| `claims_for_verification_clean.csv` | 20 rows × 36 cols. `paper_id`, `claim_id`, paper bibliographic info, claim text, `orig_*` extracted statistics from the paper, `pr_data_location`, `is_published_vor`, `repro_version_of_record`. **No reproduction values.** | Primary input — the work list. |
| `papers/<paper_id>.pdf` | Published-version-of-record PDFs for all 4 papers. | Read these for extraction verification. |

Filter the CSV to `is_published_vor == True` AND `repro_version_of_record == TRUE` → **17 claims across 4 papers**. These are the work units. Process them paper-by-paper (one paper, one report, all of that paper's claims handled together).
...
- `repro_version_of_record` — claim-level: this is the canonical reproduction-attempt row. For claims with multiple analyst attempts, exactly one row is TRUE per claim; that's the row whose `orig_*` extraction defines the comparison target. Some single-attempt claims are also flagged FALSE because they were not canonicalized. Excluding those keeps the audit aligned with the comparison set.
...
- Paper text says "approximately 0.45" (2 decimals); you reproduced 0.4463 → round to 0.45 → equals 0.45 → **precise.** (Do not invent a finer-precision target by going to a figure when the text gives a round number — see worked example below.)
...
Paper text: *"the coefficient is approximately 0.45."*
Paper's coefficient plot (a separate figure): visual reading via graphreader is ~0.44.
Author's regression code, run on author's data: coefficient = 0.4463.

**Correct application of the rubric:**
- Original value = 0.45 (from text, 2 decimals).
- Reproduced 0.4463 → rounds to 0.45 at 2 decimals → equals 0.45 → **precise.**

**Incorrect application** (the cautionary case): use graphreader to extract 0.44 from the figure, then defend a `precise` verdict by arguing 0.446 ≠ 0.45 if rounded up. This inverts the rounding rule (round to *paper's* precision, not analyst's), introduces graphreader measurement error into the comparison target, and bypasses the canonical text-derived value. The verdict still might come out `precise` but for the wrong reason — and the same path on a less forgiving claim could flip the verdict.
...
### Worked patterns
- **Author do-file references a wrong filter clause / variable name.** If the posted do-file has `treatment_wave==2` but the variable that produces the paper's N is `consented==1`, *running unpatched* (with the wrong filter) is your headline. Patch in the diagnostic section, report a "verdict if patched."
- **Author's released data has extra rows the paper excluded.** If the released `.dta` has rows with `land_area > 5000` that the paper's printed N excludes but the posted code does not contain that filter, *do not add the filter for the headline verdict.* Headline keeps the unpatched-N result; diagnostic section reports what the filter does.
...
Create `independent_reproductions/<paper_id>_<firstauthorlast>_<year>/` (e.g. `independent_reproductions/Qx9z_smith_2014/`). All work for this paper goes here. Copy `papers/<paper_id>.pdf` in. If the PDF is missing, mark the paper blocked and move on; do not invent paper content.
...
- **Reasoning:** why you think it's a bug (e.g. "filter clause `treatment_wave==2` references a variable that doesn't exist; `consented==1` is the variable that produces the paper's N").
